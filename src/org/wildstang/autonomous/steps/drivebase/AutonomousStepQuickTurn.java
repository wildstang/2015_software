...
public class AutonomousStepQuickTurn extends AutonomousStep {
	...
	public void initialize() {
		...
		angle = ((DriveBase) SubsystemContainer.getInstance().getSubsystem(
				SubsystemContainer.DRIVE_BASE_INDEX)).getGyroAngle()
				+ value;
		((DriveBase) SubsystemContainer.getInstance().getSubsystem(
				SubsystemContainer.DRIVE_BASE_INDEX)).setThrottleValue(0);
		((DriveBase) SubsystemContainer.getInstance().getSubsystem(
				SubsystemContainer.DRIVE_BASE_INDEX))
				.overrideHeadingValue(value < 0 ? 0.6 : -0.6);
	...
	public void update() {
		...
		double gyroAngle = ((DriveBase) SubsystemContainer.getInstance()
