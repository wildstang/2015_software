...
public class BallHandler extends Subsystem implements IObserver {
	...
	public static final ArmPreset DEFAULT_POSITION = new ArmPreset(0, 0,
			"DefaultPosition");
	public static final ArmPreset CATAPULT_TENSION_PRESET = new ArmPreset(20,
			20, "TensionPreset");

	public static final ArmPreset FRONT_ARM_ONLY_90 = new ArmPreset(90,
			ArmPreset.IGNORE_VALUE, "FrontArmOnly90");
	public static final ArmPreset FRONT_ARM_ONLY_45 = new ArmPreset(45,
			ArmPreset.IGNORE_VALUE, "FrontArmOnly45");
	public static final ArmPreset FRONT_ARM_ONLY_135 = new ArmPreset(135,
			ArmPreset.IGNORE_VALUE, "FrontArmOnly135");
	public static final ArmPreset FRONT_ARM_ONLY_NEG15 = new ArmPreset(-15,
			ArmPreset.IGNORE_VALUE, "FrontArmOnly-15");
	public static final ArmPreset FRONT_ARM_ONLY_0 = new ArmPreset(0,
			ArmPreset.IGNORE_VALUE, "FrontArmOnly0");
	public static final ArmPreset BACK_ARM_ONLY_45 = new ArmPreset(
			ArmPreset.IGNORE_VALUE, 45, "BackArmOnly45");
	public static final ArmPreset BACK_ARM_ONLY_90 = new ArmPreset(
			ArmPreset.IGNORE_VALUE, 90, "BackArmOnly90");
	public static final ArmPreset BACK_ARM_ONLY_135 = new ArmPreset(
			ArmPreset.IGNORE_VALUE, 135, "BackArmOnly135");
	public static final ArmPreset BACK_ARM_ONLY_NEG15 = new ArmPreset(
			ArmPreset.IGNORE_VALUE, -15, "BackArmOnly-15");
	public static final ArmPreset BACK_ARM_ONLY_0 = new ArmPreset(
			ArmPreset.IGNORE_VALUE, 0, "BackArmOnly0");
	...
	protected Debouncer frontArmCalibrationDebouncer,
			backArmCalibrationDebouncer;

	public BallHandler(String name) {
		super(name);

		this.frontArm = new Arm(OutputManager.FRONT_ARM_VICTOR_INDEX,
				OutputManager.FRONT_ARM_ROLLER_VICTOR_INDEX,
				InputManager.FRONT_ARM_POT_INDEX, true);
		this.backArm = new Arm(OutputManager.BACK_ARM_VICTOR_INDEX,
				OutputManager.BACK_ARM_ROLLER_VICTOR_INDEX,
				InputManager.BACK_ARM_POT_INDEX, false);
		...
		Subject subject = InputManager.getInstance()
				...
				.getSubject(JoystickAxisEnum.MANIPULATOR_BACK_ARM_CONTROL);
		subject.attach(this);

		subject = InputManager.getInstance()
				.getOiInput(InputManager.MANIPULATOR_JOYSTICK_INDEX)
				.getSubject(JoystickAxisEnum.MANIPULATOR_FRONT_ARM_CONTROL);
		subject.attach(this);
	...
	public void update() {
		...
		String frontString = frontValue == ArmRollerEnum.INTAKE ? "Forward"
				: (frontValue == ArmRollerEnum.OUTPUT ? "Reverse" : "Off");
		String backString = backValue == ArmRollerEnum.INTAKE ? "Forward"
				: (backValue == ArmRollerEnum.OUTPUT ? "Reverse" : "Off");
		...
		this.frontArmCalibrationDebouncer
				.update(InputManager
						.getInstance()
						.getSensorInput(
								InputManager.FRONT_ARM_CALIBRATION_SWITCH_INDEX)
						.get());
		this.backArmCalibrationDebouncer.update(InputManager.getInstance()
				.getSensorInput(InputManager.BACK_ARM_CALIBRATION_SWITCH_INDEX)
				.get());

		SmartDashboard.putNumber("Current Front Arm Angle",
				frontArm.getCurrentAngle());
		SmartDashboard.putNumber("Wanted Front Arm Angle",
				frontArm.getWantedAngle());
		SmartDashboard.putNumber("Current Back Arm Angle",
				backArm.getCurrentAngle());
		SmartDashboard.putNumber("Wanted Back Arm Angle",
				backArm.getWantedAngle());
		...
		SmartDashboard.putBoolean("Front Arm At Zero",
				!((Boolean) this.frontArmCalibrationDebouncer
						.getDebouncedValue()).booleanValue());
		SmartDashboard.putBoolean("Back Arm At Zero",
				!((Boolean) this.backArmCalibrationDebouncer
						.getDebouncedValue()).booleanValue());
	}
	...
	public void acceptNotification(Subject subjectThatCaused) {
		if (subjectThatCaused.getType() == JoystickButtonEnum.MANIPULATOR_BUTTON_5) {
			...
			if (!((Boolean) this.frontArmCalibrationDebouncer
					.getDebouncedValue()).booleanValue()) {
			...
			// The limit switches return false when pressed (Electrical)
			if (!((Boolean) this.backArmCalibrationDebouncer
					.getDebouncedValue()).booleanValue()) {
	...
	public void setArmPreset(ArmPreset preset) {
		...
		if (frontArmPreset <= frontArm.getHighBound()
				&& frontArmPreset >= frontArm.getLowBound()) {
			this.frontArm.setToAngle(frontArmPreset);
		}
		if (backArmPreset <= backArm.getHighBound()
				&& backArmPreset >= backArm.getLowBound()) {
