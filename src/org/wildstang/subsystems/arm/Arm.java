 ...
 */
public class Arm {
	protected static final BooleanConfigFileParameter DISABLE_PID_COMPLETELY_CONFIG = new BooleanConfigFileParameter(
			Arm.class.getName(), "DisablePIDCompletely", false);
	protected static final BooleanConfigFileParameter DISABLE_PID_INIT_CONFIG = new BooleanConfigFileParameter(
			Arm.class.getName(), "DisablePIDInInit", false);
	protected static final DoubleConfigFileParameter ARM_VICTOR_SPEED_DILUTION_CONFIG = new DoubleConfigFileParameter(
			Arm.class.getName(), "ArmVictorSpeedDilution", 0.5);
	protected static boolean disablePidCompletely = DISABLE_PID_COMPLETELY_CONFIG
			.getValue(), diablePidInit = DISABLE_PID_INIT_CONFIG.getValue();
	protected static double armVictorSpeedDilution = ARM_VICTOR_SPEED_DILUTION_CONFIG
			.getValue();

	protected final DoubleConfigFileParameter TOP_VOLTAGE_VALUE_CONFIG,
			BOTTOM_VOLTAGE_VALUE_CONFIG, ROLLER_FORWARD_SPEED_CONFIG,
			ROLLER_REVERSE_SPEED_CONFIG;
	protected final IntegerConfigFileParameter HIGH_BOUND_CONFIG,
			LOW_BOUND_CONFIG;
	...
	public Arm(int victorAngleIndex, int armRollerVictorIndex, int potIndex,
			boolean front) {
		...
		System.out.println("ARM_ROLLER_VICTOR_INDEX: "
				+ ARM_ROLLER_VICTOR_INDEX);
		System.out.println("END ARM CONSTRUCTOR");

		TOP_VOLTAGE_VALUE_CONFIG = new DoubleConfigFileParameter(this
				.getClass().getName(), (front ? "Front" : "Back")
				+ ".TopVoltageValue", 5.1);
		BOTTOM_VOLTAGE_VALUE_CONFIG = new DoubleConfigFileParameter(this
				.getClass().getName(), (front ? "Front" : "Back")
				+ ".BottomVoltageValue", 0.0);
		ROLLER_FORWARD_SPEED_CONFIG = new DoubleConfigFileParameter(this
				.getClass().getName(), (front ? "Front" : "Back")
				+ ".RollerForwardSpeed", 0.5);
		ROLLER_REVERSE_SPEED_CONFIG = new DoubleConfigFileParameter(this
				.getClass().getName(), (front ? "Front" : "Back")
				+ ".RollerReverseSpeed", -0.5);
		HIGH_BOUND_CONFIG = new IntegerConfigFileParameter(Arm.class.getName(),
				(front ? "Front" : "Back") + ".HighAngle", 359);
		LOW_BOUND_CONFIG = new IntegerConfigFileParameter(Arm.class.getName(),
				(front ? "Front" : "Back") + ".LowAngle", -20);
		...
		this.pidInput = new ArmPotPidInput(potIndex, topVoltage, bottomVoltage,
				highBound, lowBound);
		this.pid = new PidController(this.pidInput, new ArmVictorPidOutput(
				victorAngleIndex), front ? "FrontArmPid" : "BackArmPid");
		this.pid.disable();
	}
	...
	public void setVictor(double speed) {
		...
		OutputManager.getInstance().getOutput(VICTOR_ANGLE_INDEX)
				.set(new Double(speed));
	}

	public double getVictorSpeed() {
		return ((Double) OutputManager.getInstance()
				.getOutput(VICTOR_ANGLE_INDEX).get()).doubleValue();
	}
	...
	public void init() {
		...
		IOutput o = OutputManager.getInstance().getOutput(
				ARM_ROLLER_VICTOR_INDEX);
		System.out.println("arm roller victor indexe: "
				+ ARM_ROLLER_VICTOR_INDEX);
	...
	public void update() {
		...
		double currentVoltage = ((Double) InputManager.getInstance()
				.getSensorInput(POT_INDEX).get()).doubleValue();

		SmartDashboard.putNumber("Pot Voltage " + (front ? "Front" : "Back"),
				currentVoltage);
		...
		if (rollerValue == ArmRollerEnum.INTAKE) {
			OutputManager.getInstance().getOutput(ARM_ROLLER_VICTOR_INDEX)
					.set(new Double(rollerReverseSpeed));
		} else if (rollerValue == ArmRollerEnum.OFF) {
			OutputManager.getInstance().getOutput(ARM_ROLLER_VICTOR_INDEX)
					.set(new Double(0.0));
		} else if (rollerValue == ArmRollerEnum.OUTPUT) {
			OutputManager.getInstance().getOutput(ARM_ROLLER_VICTOR_INDEX)
					.set(new Double(rollerForwardSpeed));
		}

		SmartDashboard.putString(this.pid.getName(), this.pid.getState()
				.toString());
	}
	...
	public double getPotVoltage() {
		return ((Double) InputManager.getInstance()
				.getSensorInput(this.POT_INDEX).get()).doubleValue();
	}
