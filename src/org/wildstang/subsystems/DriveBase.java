...
public class DriveBase extends Subsystem implements IObserver {
	...
	public DriveBase(String name) {
		...
		WHEEL_DIAMETER_config = new DoubleConfigFileParameter(this.getClass()
				.getName(), "wheel_diameter", 6);
		TICKS_PER_ROTATION_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "ticks_per_rotation", 360.0);
		THROTTLE_LOW_GEAR_ACCEL_FACTOR_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "throttle_low_gear_accel_factor",
				0.250);
		HEADING_LOW_GEAR_ACCEL_FACTOR_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "heading_low_gear_accel_factor",
				0.500);
		THROTTLE_HIGH_GEAR_ACCEL_FACTOR_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "throttle_high_gear_accel_factor",
				0.125);
		HEADING_HIGH_GEAR_ACCEL_FACTOR_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "heading_high_gear_accel_factor",
				0.250);
		MAX_HIGH_GEAR_PERCENT_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "max_high_gear_percent", 0.80);
		ENCODER_GEAR_RATIO_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "encoder_gear_ratio", 7.5);
		DEADBAND_config = new DoubleConfigFileParameter(this.getClass()
				.getName(), "deadband", 0.05);
		SLOW_TURN_FORWARD_SPEED_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "slow_turn_forward_speed", 0.16);
		SLOW_TURN_BACKWARD_SPEED_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "slow_turn_backward_speed", -0.19);
		FEED_FORWARD_VELOCITY_CONSTANT_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "feed_forward_velocity_constant",
				1.00);
		FEED_FORWARD_ACCELERATION_CONSTANT_config = new DoubleConfigFileParameter(
				this.getClass().getName(),
				"feed_forward_acceleration_constant", 0.00018);
		MAX_ACCELERATION_DRIVE_PROFILE_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "max_acceleration_drive_profile",
				600.0);
		MAX_SPEED_INCHES_LOWGEAR_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "max_speed_inches_lowgear", 90.0);
		DECELERATION_VELOCITY_THRESHOLD_config = new DoubleConfigFileParameter(
				this.getClass().getName(), "deceleration_velocity_threshold",
				48.0);
		DECELERATION_MOTOR_SPEED_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "deceleration_motor_speed", 0.3);
		STOPPING_DISTANCE_AT_MAX_SPEED_LOWGEAR_config = new DoubleConfigFileParameter(
				this.getClass().getName(),
				"stopping_distance_at_max_speed_lowgear", 10.0);
		DRIVE_OFFSET_config = new AutonomousDoubleConfigFileParameter(
				"DriveOffset", 1.00);
		USE_LEFT_SIDE_FOR_OFFSET_config = new AutonomousBooleanConfigFileParameter(
				"UseLeftDriveForOffset", true);
		QUICK_TURN_CAP_config = new DoubleConfigFileParameter(this.getClass()
				.getName(), "quick_turn_cap", 10.0);
		QUICK_TURN_ANTITURBO_config = new DoubleConfigFileParameter(this
				.getClass().getName(), "quick_turn_antiturbo", 10.0);
	...
	public void init() {
		...
		OutputManager.getInstance()
				.getOutput(OutputManager.LEFT_DRIVE_SPEED_INDEX)
				.set(new Double(0.0));
		OutputManager.getInstance()
				.getOutput(OutputManager.RIGHT_DRIVE_SPEED_INDEX)
				.set(new Double(0.0));
		OutputManager.getInstance()
				.getOutput(OutputManager.LEFT_DRIVE_SPEED_INDEX).update();
		OutputManager.getInstance()
				.getOutput(OutputManager.RIGHT_DRIVE_SPEED_INDEX).update();
		InputManager.getInstance()
				.getOiInput(InputManager.DRIVER_JOYSTICK_INDEX)
				.set(JoystickAxisEnum.DRIVER_THROTTLE, new Double(0.0));
		InputManager.getInstance()
				.getOiInput(InputManager.DRIVER_JOYSTICK_INDEX)
				.set(JoystickAxisEnum.DRIVER_HEADING, new Double(0.0));
		InputManager.getInstance()
				.getOiInput(InputManager.DRIVER_JOYSTICK_INDEX).update();
		// Clear encoders
		resetLeftEncoder();
		resetRightEncoder();
	}
	...
	public void update() {
		...
		if (true == motionProfileActive) {
			...
			continuousAccelerationFilter.calculateSystem(distance_remaining,
					currentProfileV, goal_velocity,
					MAX_ACCELERATION_DRIVE_PROFILE, MAX_SPEED_INCHES_LOWGEAR,
					deltaTime);
			...
			SmartDashboard.putNumber("Speed PID Error",
					driveSpeedPid.getError());
			...
			SmartDashboard.putNumber("Motion Profile Throttle",
					driveBaseThrottleValue);
			...
			throttleValue = ((Double) ((InputManager.getInstance()
					.getOiInput(InputManager.DRIVER_JOYSTICK_INDEX)))
					.get(JoystickAxisEnum.DRIVER_THROTTLE)).doubleValue();
			headingValue = ((Double) ((InputManager.getInstance()
					.getOiInput(InputManager.DRIVER_JOYSTICK_INDEX)))
					.get(JoystickAxisEnum.DRIVER_HEADING)).doubleValue();
			...
			SmartDashboard.putBoolean("Shifter State",
					shifterFlag.equals(DoubleSolenoid.Value.kReverse));
			...
			OutputManager.getInstance().getOutput(OutputManager.SHIFTER_INDEX)
					.set(new Integer(shifterFlag.value));
		}
	...
	public void updateDriveMotors(boolean useDriveOffset) {
		...
		if (Math.abs(driveBaseHeadingValue) > 0.05) {
			...
			// is banana
			angularPower = driveBaseThrottleValue * driveBaseHeadingValue
					* HEADING_SENSITIVITY;
		}
		...
		OutputManager.getInstance()
				.getOutput(OutputManager.LEFT_DRIVE_SPEED_INDEX)
				.set(new Double(leftMotorSpeed));
		OutputManager.getInstance()
				.getOutput(OutputManager.RIGHT_DRIVE_SPEED_INDEX)
				.set(new Double(rightMotorSpeed));
	}
	...
	public double getLeftDistance() {
		double distance = 0.0;
		distance = (leftDriveEncoder.get() / (TICKS_PER_ROTATION * ENCODER_GEAR_RATIO))
				* 2.0 * Math.PI * (WHEEL_DIAMETER / 2.0);
		return distance;
	}

	public double getRightDistance() {
		double distance = 0.0;
		distance = (rightDriveEncoder.get() / (TICKS_PER_ROTATION * ENCODER_GEAR_RATIO))
				* 2.0 * Math.PI * (WHEEL_DIAMETER / 2.0);
		return distance;
	}
	...
	public void disableSpeedPidControl() {
		...
		resetSpeedPid();
		Logger.getLogger().debug(this.getClass().getName(),
				"disableSpeedPidControl", "Speed PID is disabled");
	}
	...
	public void notifyConfigChange() {
		...
		THROTTLE_LOW_GEAR_ACCEL_FACTOR = THROTTLE_LOW_GEAR_ACCEL_FACTOR_config
				.getValue();
		HEADING_LOW_GEAR_ACCEL_FACTOR = HEADING_LOW_GEAR_ACCEL_FACTOR_config
				.getValue();
		THROTTLE_HIGH_GEAR_ACCEL_FACTOR = THROTTLE_HIGH_GEAR_ACCEL_FACTOR_config
				.getValue();
		HEADING_HIGH_GEAR_ACCEL_FACTOR = HEADING_HIGH_GEAR_ACCEL_FACTOR_config
				.getValue();
		...
		FEED_FORWARD_VELOCITY_CONSTANT = FEED_FORWARD_VELOCITY_CONSTANT_config
				.getValue();
		FEED_FORWARD_ACCELERATION_CONSTANT = FEED_FORWARD_ACCELERATION_CONSTANT_config
				.getValue();
		MAX_ACCELERATION_DRIVE_PROFILE = MAX_ACCELERATION_DRIVE_PROFILE_config
				.getValue();
		MAX_SPEED_INCHES_LOWGEAR = MAX_SPEED_INCHES_LOWGEAR_config.getValue();
		DEADBAND = DEADBAND_config.getValue();
		DECELERATION_VELOCITY_THRESHOLD = DECELERATION_VELOCITY_THRESHOLD_config
				.getValue();
		DECELERATION_MOTOR_SPEED = DECELERATION_MOTOR_SPEED_config.getValue();
		STOPPING_DISTANCE_AT_MAX_SPEED_LOWGEAR = STOPPING_DISTANCE_AT_MAX_SPEED_LOWGEAR_config
				.getValue();
	...
	public void acceptNotification(Subject subjectThatCaused) {
		if (subjectThatCaused.getType() == JoystickButtonEnum.DRIVER_BUTTON_8) {
			...
			if (((BooleanSubject) subjectThatCaused).getValue() == true) {
				shifterFlag = shifterFlag.equals(DoubleSolenoid.Value.kForward) ? DoubleSolenoid.Value.kReverse
	...
	private double getStoppingDistanceFromDistanceToMove(double distance) {
		if (Math.abs(distance) > 40.0) {
			...
			return (STOPPING_DISTANCE_AT_MAX_SPEED_LOWGEAR - ((3.0f / 15.0f) * (40 - Math
					.abs(distance))));
		}
	}
